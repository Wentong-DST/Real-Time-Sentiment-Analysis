...
Num_RE = mycompile(regex_or(*[NumNum, NumberWithCommas, Num]))
...
Repeating_RE = mycompile(r"([a-zA-Z])\1\1+")
...
def simple_tokenize(text, lowercase=0):
  ...
  # replace URL, NUMBERs and MENTION with special tokens
  if Mention_RE.search(s):
    s = Mention_RE.sub('_MENTION_', s)
...
def post_process(pre_toks):
  ...
  for tok in pre_toks:
    ...
    if m:
      ...
      if Repeating_RE.search(tok):
        tok = Repeating_RE.sub(r"\1"*2, tok)
